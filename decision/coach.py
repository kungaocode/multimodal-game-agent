...
def _parse_resource_buildings(
    items: Any,
    image_size: tuple[int, int] | None,
    viewed_size: tuple[int, int] | None,
) -> list[tuple[str, tuple[int, int]]]:
    """解析教练枚举的「全部可见敌方资源建筑」，非法项丢弃。

    与 corrected_action.coords 同一套坐标判据：center 在 [0,1] 视为归一化坐标
    乘 image_size 转像素；否则按旧版 viewed→orig 画布缩放。过滤条件：
    type 必须是 resource_policy.RESOURCE_NAMES 的 6 类之一；center 缺失/非数值/
    归一化坐标越界 → 丢弃。
    """
    ...
    for item in items:
        ...
        if values[0] <= 1.0 and values[1] <= 1.0:
            point = _normalize_point(values, image_size)
        else:
            point = _scale_legacy_pixel_coords(values, viewed_size, image_size)
        if point is None:
            continue
...
def build_coach_prompt(
    ...
    lines.append(
        ...
        "  ],\n"
        '  "resource_buildings": [\n'
        '    {"type": "<6类建筑名>", "center": [0.0, 1.0]}  // 战斗中本帧视野内可见的敌方资源建筑；center 归一化\n'
        "  ],\n"
        '  "reasoning": "<简要理由>"\n'
        "}\n"
        "\n"
        ...
        "可见且对手不值得打，tap 下一个。\n"
        "- 战斗中: 红色「结束战斗」按钮整场常驻，它的出现绝不代表战斗结束，"
        "绝不因为看到它、有倒计时或某一帧视野暂无资源就点结束/返回。\n"
        "- 战斗中: 只要视野内还有 6 类敌方资源建筑"
        "(金矿/圣水收集器/储金罐/圣水瓶/暗黑重油罐/暗黑重油钻井) 中任意一个"
        "且该资源类型自己未满，唯一正确动作就是 deploy 到其中一个（可获取的）；"
        "绝不 approve wait，更不点结束。\n"
        "- resource_buildings: 战斗中每一帧都把本帧视野内可见的全部敌方资源建筑都列出来"
        "（不只单个目标，供算法打分排序）；一帧看不到任何资源建筑就写空列表 []。\n"
        "- 只有视野内已不再有任何 6 类敌方资源建筑（打光或画面切走）时，才允许 tap "
        "「返回按钮」或「结束战斗」结束战斗。\n"
        "- 战斗结束: tap 返回按钮.\n"
        ...
        "- For resource_buildings, center = [x/width, y/height] (normalized), and type must be "
        "one of the 6 resource-building names above.\n"
...
class CoachVerdict:
    ...
    # 教练本帧看到的「全部可见敌方资源建筑 (6 类名, 像素中心)」，
    # 回填进 signals.enemy_resources 供资源算法打分/判空，避免只给单个目标。
...
class CoachReviewer:
    ...
    def _parse_verdict(
        ...
        verdict.resource_buildings = _parse_resource_buildings(
            parsed.get("resource_buildings"), image_size, viewed_size
        )
        return verdict
